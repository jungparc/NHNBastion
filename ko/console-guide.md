...
> [주의]
> 최초 생성된 웹 터미널이 생성된 VPC가 인터넷게이트웨이가 없는 경우에는 OBS 백업이 지원되지 않습니다.
...
#### 파일 전송
우측 화살표 버튼을 이용하여 파일 네비게이터를 실행할 수 있습니다. 파일 네비게이터를 통하여 원하는 경로의 파일을 업로드하거나 다운로드 할 수 있습니다.

>[주의]
>최초 접속한 OS계정의 권한을 기준으로 파일 전송이 가능하며, su 명령어를 통한 계정 변경은 반영되지 않습니다.
